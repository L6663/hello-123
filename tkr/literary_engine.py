...
def verify_literary_engine(output_directory: str | Path) -> LiteraryVerification:
    ...
    try:
        ...
        # Reparse every typed JSONL artifact so malformed records cannot hide behind hashes.
        type_map = {
            "chapters.jsonl": "chapter",
            "evidence-anchors.jsonl": "evidence",
            "entities.jsonl": "entity",
            "assertions.jsonl": "assertion",
            "relationships.jsonl": "relationship",
            "events.jsonl": "event",
            "revisions.jsonl": "revision",
        }
        for name, record_type in type_map.items():
            for row in _load_jsonl(root / name, name):
                try:
