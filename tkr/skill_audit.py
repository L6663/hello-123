"""Stage 5 product-layout audit and runtime environment doctor."""
...
SKILL_AUDIT_VERSION = "5.9.0-stage5"
...
def _profile_from_path(path: Path):
    return load_engineering_profile(path)
...
def audit_skill_layout(root: str | Path | None = None) -> SkillAuditReport:
    ...
    for directory in _REQUIRED_DIRECTORIES:
        ...
        if path.is_symlink() or not path.is_dir():
            add("REQUIRED_DIRECTORY_MISSING", "blocker", directory, "required directory is missing or unsafe")
    for relative in _REQUIRED_FILES:
        path = selected / relative
        if path.is_symlink() or not path.is_file():
            add("REQUIRED_FILE_MISSING", "blocker", relative, "required file is missing or unsafe")
            continue
        checked += 1
        try:
            _safe_text(path)
        ...
        try:
            ...
            for marker in _REQUIRED_SKILL_MARKERS:
                if marker not in skill_text:
                    add("SKILL_CONTRACT_SECTION_MISSING", "blocker", "SKILL.md", marker)
        ...
        try:
            ...
            for script in _REQUIRED_SCRIPTS:
                if f"{script} =" not in project_text:
                    add("CONSOLE_SCRIPT_MISSING", "blocker", "pyproject.toml", script)
            for required in ("SKILL.md", "profiles/balanced.json", "examples/minimal-corpus.txt"):
                if required not in project_text:
                    add("PACKAGE_DATA_DECLARATION_MISSING", "blocker", "pyproject.toml", required)
        except (OSError, UnicodeError, ValueError):
            pass
    ...
    if schema_root.is_dir() and not schema_root.is_symlink():
        for path in sorted(schema_root.glob("*.json")):
            if path.is_symlink() or not path.is_file():
                add("SCHEMA_FILE_UNSAFE", "blocker", path.relative_to(selected).as_posix(), "unsafe schema file")
                continue
            checked += 1
            try:
                ...
                add("SCHEMA_INVALID", "blocker", path.relative_to(selected).as_posix(), str(exc))
    if schema_count < 20:
        add("SCHEMA_SET_INCOMPLETE", "blocker", "schemas", f"only {schema_count} schemas were valid")
    ...
    if profile_root.is_dir() and not profile_root.is_symlink():
        ...
        for path in sorted(profile_root.glob("*.json")):
            ...
            try:
                ...
                add("PROFILE_INVALID", "blocker", path.relative_to(selected).as_posix(), str(exc))
    ...
    if questions.is_file() and not questions.is_symlink():
        try:
            for number, line in enumerate(_safe_text(questions).splitlines(), start=1):
                if not line.strip():
            ...
            add("EXAMPLE_QUESTIONS_INVALID", "blocker", "examples/questions.jsonl", str(exc))

    unique = tuple(findings)
    status = "passed" if not any(item.severity in {"blocker", "high"} for item in unique) else "failed"
...
def doctor_environment(root: str | Path | None = None) -> SkillDoctorReport:
    checks: list[DoctorCheck] = []

    def check(name: str, passed: bool, detail: str) -> None:
        checks.append(DoctorCheck(name, "passed" if passed else "failed", detail))
    ...
    check("sqlite_available", bool(sqlite3.sqlite_version), sqlite3.sqlite_version)
    fts5 = _fts5_available()
    check("sqlite_fts5", fts5, "available" if fts5 else "LIKE fallback remains available")
    try:
        with tempfile.TemporaryDirectory(prefix="tkr-doctor-") as directory:
            probe = Path(directory) / "probe"
            probe.write_text("ok", encoding="utf-8")
            writable = probe.read_text(encoding="utf-8") == "ok"
        check("temporary_storage", writable, "atomic temporary storage is writable")
    ...
    try:
        ...
        check("skill_layout", audit.passed, f"{audit.checked_file_count} files; {audit.finding_count} findings")
    except (OSError, UnicodeError, ValueError) as exc:
