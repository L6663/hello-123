...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="tkr-gold-benchmark", description="Run or verify immutable Gold Benchmark gates for strict QA.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        if args.command == "run":
            report = evaluate_gold_benchmark(args.database, args.gold, profile=args.profile, report_path=args.index_report)
            payload = report.to_dict()
            if args.output is not None:
                _write_json(args.output, payload)
            print(json.dumps(payload, ensure_ascii=False, sort_keys=True))
            return 0 if report.passed else 2
        result = verify_benchmark_report(args.database, args.gold, args.benchmark_report, index_report_path=args.index_report)
